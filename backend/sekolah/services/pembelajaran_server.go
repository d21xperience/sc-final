...
func (s *PembelajaranServiceServer) GetPembelajaran(ctx context.Context, req *pb.GetPembelajaranRequest) (*pb.GetPembelajaranResponse, error) {
	...
	joins := []string{
		...
		"JOIN tabel_ptk_terdaftar ON tabel_ptk_terdaftar.ptk_terdaftar_id = tabel_pembelajaran.ptk_terdaftar_id",
	}
	preloads := []string{"Kelas", "Mapel", "PTKTerdaftar"}

	orderBy := []string{"rombongan_belajar_id ASC"} // Hindari duplikasi
	anggotaRombelModel, err := s.repo.FindWithPreloadAndJoinsOrigin(ctx, schemaName, joins, preloads, conditions, orderBy)
	if err != nil {
		return nil, err
	}

	banyakPembelajaranList := utils.ConvertModelsToPB(anggotaRombelModel, func(anggota models.Pembelajaran) *pb.Pembelajaran {

		return &pb.Pembelajaran{
			RombonganBelajarId: anggota.RombonganBelajarId.String(),
			MataPelajaranId:    int32(anggota.MataPelajaranId),
			SemesterId:         anggota.SemesterId,
